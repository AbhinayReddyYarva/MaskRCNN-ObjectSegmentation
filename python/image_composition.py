...
class MaskJsonUtils():
    ...
    def write_masks_to_json(self):
        ...
        with open(str(output_file_path), 'w+') as json_file:
...
class ImageComposition():
    ...
    def _validate_and_process_args(self, args):
        ...
        if args.output_type is None:
            ...
            if args.output_type[0] != '.':
                self.output_type = '.{}'.format(args.output_type)
            assert self.output_type in self.allowed_output_types, 'output_type is not supported: {}'.format(self.output_type)
    ...
    def _validate_and_process_input_directory(self):
        ...
        assert self.input_dir.exists(), 'input_dir does not exist: {}'.format(args.input_dir)
    ...
    def _validate_and_process_foregrounds(self):
        ...
        for super_category_dir in self.foregrounds_dir.iterdir():
            if not super_category_dir.is_dir():
                warnings.warn('file found in foregrounds directory (expected super-category directories), ignoring: {}'.format(super_category_dir))
                continue

            # This is a super category directory
            for category_dir in super_category_dir.iterdir():
                if not category_dir.is_dir():
                    warnings.warn('file found in super category directory (expected category directories), ignoring: {}'.format(category_dir))
                    continue

                # This is a category directory
                for image_file in category_dir.iterdir():
                    if not image_file.is_file():
                        warnings.warn('a directory was found inside a category directory, ignoring: {}'.format(str(image_file)))
                        continue
                    if image_file.suffix != '.png':
                        warnings.warn('foreground must be a .png file, skipping: {}'.format(str(image_file)))
                        continue
    ...
    def _validate_and_process_backgrounds(self):
        ...
        for image_file in self.backgrounds_dir.iterdir():
            if not image_file.is_file():
                warnings.warn('a directory was found inside the backgrounds directory, ignoring: {}'.format(image_file))
                continue

            if image_file.suffix not in self.allowed_background_types:
                warnings.warn('background must match an accepted type {0}, ignoring: {1}'.format(str(self.allowed_background_types), image_file))
                continue
    ...
    def _generate_images(self):
        ...
        print('Generating {} images with masks...'.format(self.count))
        ...
        for i in tqdm(range(self.count)):
            ...
            save_filename = '{:0>{}}'.format(i, self.zero_padding) # e.g. 00000023.jpg

            # Save composite image to the images sub-directory
            composite_filename = '{0}{1}'.format(save_filename, self.output_type) # e.g. 00000023.jpg
            ...
            mask_filename = '{}.png'.format(save_filename) # masks are always png to avoid lossy compression
    ...
    def _compose_images(self, foregrounds, background_path):
        ...
        assert max_crop_x_pos >= 0, 'desired width, {0}, is greater than background width, {1}, for {2}'.format(self.width, bg_width, str(background_path))
        assert max_crop_y_pos >= 0, 'desired height, {0}, is greater than background height, {2}, for {3}'.format(self.height, bg_height, str(background_path))
        ...
        for fg in foregrounds:
            ...
            'foreground {0} is too big ({1}x{2}) for the requested output size ({3}x{4}), check your input parameters'.format(fg_path, fg_image.size[0], fg_image.size[1], self.width, self.height)
    ...
    def _transform_foreground(self, fg, fg_path):
        ...
        assert np.any(fg_alpha == 0), 'foreground needs to have some transparency: {}'.format(str(fg_path))
    ...
    def _create_info(self):
        ...
        now = datetime.now()
        info['year'] = now.year
        info['date_created'] = '{foo1:0>{pad}}/{foo2:0>{pad}}/{foo3}'.format(foo1=now.month, foo2=now.day, foo3=now.year, pad=2)
