    ...
    def _create_annotations(self):
        ...
        for key, mask in self.isolated_masks.items():
            ...
            if not self.category_ids.get(key):
                print('category color not found: {}; check for missing category or antialiasing'.format(key))
                continue
...
class CocoJsonCreator():
    def validate_and_process_args(self, args):
        ...
        if not (mask_definition_file.exists and mask_definition_file.is_file()):
            raise FileNotFoundError('mask definition file was not found: {}'.format(mask_definition_file))

        # Load the mask definition json
        with open(str(mask_definition_file)) as json_file:
        ...
        if not (dataset_info_file.exists() and dataset_info_file.is_file()):
            raise FileNotFoundError('dataset info file was not found: {}'.format(dataset_info_file))

        # Load the dataset info json
        with open(str(dataset_info_file)) as json_file:
    ...
    def create_images_and_annotations(self, category_ids_by_name):
        ...
        print('Processing {} mask definitions...'.format(mask_count))
    ...
    def main(self, args):
        ...
        print('Annotations successfully written to file:\n{}'.format(output_path))
